...
void DemoWindow::OnUpdate(float deltaTime) {
    ...
    if (select_all) {
        ...
        ImGui::ListBox("", &m_selectedDemo, listbox_items, 1, 5);
...
void DemoWindow::OnFixedUpdate(float deltaTime) {
    ...
    bool leftDown = MouseButtonDown(MOUSE_LEFT);
    bool middleDown = MouseButtonDown(MOUSE_MIDDLE);
    bool rightDown = MouseButtonDown(MOUSE_RIGHT);
