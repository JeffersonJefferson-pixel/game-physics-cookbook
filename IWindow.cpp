...
#if ENABLE_USER_DATA
    #define I_WINDOW_CPP_DEFAULT_LENGTH 5
    #define I_WINDOW_GENERIC_EXPAND_BY 10
...
void CleanupMemory(IWindow* window) {
    ...
    if (window->m_vGenericIntegerNames != 0) {
        for (int i = 0; i < window->m_nGenericIntegerCount; ++i) {
            if (window->m_vGenericIntegerNames[i] != 0) {
...
bool IWindow::GetFullScreen() {
    ...
    void IWindow::InitGenericIntegers() {
        ...
        m_nGenericIntegerLength = I_WINDOW_CPP_DEFAULT_LENGTH;

        m_vGenericIntegerValues = (int*)malloc(sizeof(int) * I_WINDOW_CPP_DEFAULT_LENGTH);
        memset(m_vGenericIntegerValues, 0, sizeof(int) * I_WINDOW_CPP_DEFAULT_LENGTH);

        m_vGenericIntegerNames = (char**)malloc(sizeof(char*) * I_WINDOW_CPP_DEFAULT_LENGTH);
        memset(m_vGenericIntegerNames, 0, sizeof(char*) * I_WINDOW_CPP_DEFAULT_LENGTH);
    }
